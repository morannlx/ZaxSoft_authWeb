...
class CampusNetworkAuth(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.logout_button = ttk.Button(button_frame, text="下线", command=self.logout, bootstyle=WARNING,
                                        state=DISABLED)
        self.logout_button.pack(side=LEFT, padx=10)
        self.get_list_button = ttk.Button(button_frame, text="获取在线设备", command=self.get_online_devices,
                                          bootstyle=INFO, state=DISABLED)
        self.get_list_button.pack(side=LEFT, padx=10)
        self.clear_mac_button = ttk.Button(button_frame, text="清除MAC绑定", command=self.clear_mac_binding,
                                           bootstyle=DANGER, state=DISABLED)
        self.clear_mac_button.pack(side=LEFT, padx=10)

        # 加载配置
        if self.config:
            self.user_id_entry.insert(0, self.config.get("user_id", ""))
            self.password_entry.insert(0, self.config.get("password", ""))
            self.ua_var.set(self.config.get("ua_choose", "1"))
    ...
    def login(self):
        user_id = self.user_id_entry.get()
        password = self.password_entry.get()
        selected_ua = user_agents.get(self.ua_var.get(), user_agents['1'])
        ...
        login_success, distoken, wlanuserip = self.make_post_request(user_id, password, selected_ua)
    ...
    def fetch_online_devices(self):
        if hasattr(self, 'user_id') and hasattr(self, 'x'):
    ...
    def show_online_devices(self):
        ...
        self.device_window.geometry("800x500")  # 扩大窗口尺寸
        ...
        # 创建 Treeview 表格
        columns = ("设备ID", "系统信息", "IP地址", "MAC地址")  # 添加MAC地址列
        ...
        for device in self.online_devices:
            ...
            accountMac = device.get("accountMac", "未知MAC")  # 获取MAC地址

            # 插入行数据
            tree.insert("", "end", values=(billingId, osInfo, accountIp, accountMac))
    ...
    def refresh_online_devices(self):
        if hasattr(self, 'user_id') and hasattr(self, 'x') and hasattr(self, 'headers'):
            user_id = self.user_id
    ...
    def update_device_table(self, tree, devices):
        ...
        for device in devices:
            ...
            osInfo = device.get("osInfo", "未知系统")
            accountIp = device.get("accountIp", "0.0.0.0")
            tree.insert("", "end", values=(billingId, osInfo, accountIp))
    ...
    def logout_single_device(self, device):
        ...
        try:
            out = self.x.post(f'{host}/self/kickonline.do',
                              headers=self.headers,
                              data=json.dumps(loginout_data))
    ...
    def clear_mac_binding(self):
        """清除MAC绑定"""
        if not hasattr(self, 'cookies_login') or not self.cookies_login:
            Messagebox.show_warning("请先登录再进行操作", "提示")
            return

        try:
            # 获取MAC1和MAC2的值
            url = f'{host}/self/toRemoveMac.do?userId={self.user_id}'
            headers = {
                'User-Agent': user_agents.get(self.ua_var.get(), user_agents['1'])
            }
            r = requests.get(url, headers=headers, cookies=self.cookies_login)
            ...
            # MAC地址显示区域
            ttk.Label(content_frame, text="MAC1:", font=("Arial", 10, "bold"), bootstyle=PRIMARY).grid(row=0, column=0,
                                                                                                       sticky=W, pady=5)
            ...
            ttk.Label(content_frame, text="MAC2:", font=("Arial", 10, "bold"), bootstyle=PRIMARY).grid(row=1, column=0,
                                                                                                       sticky=W, pady=5)
    ...
    def do_clear_mac(self, clear_type):
        ...
        try:
            ...
            response = requests.post(url, headers=headers, cookies=self.cookies_login, json=data, verify=False)
